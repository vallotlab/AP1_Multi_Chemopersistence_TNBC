---
title: "QCs_&_Objects_Microarray.Rmd"
...
Output_Objects_micro_RNA = file.path(maindir, "Output","Objects", "hg38", "PDX", "RNA", "Microarray")
Output_Figs_micro_RNA = file.path(maindir, "Output","Figs", "hg38", "PDX", "RNA", "Microarray")
```

Make PCA & Hierarchical Clustering for HBCx10
```{r}
...
matrice.edger = as.matrix(microarray_mat[,-c(1)])
rownames(matrice.edger) = microarray_mat$Symbol
...
pdf(file.path(Output_Figs_micro_RNA,"PCA_HBCx10_Elbowplot.pdf"), width = 7, height = 7)
...
pdf(file.path(Output_Figs_micro_RNA,"PCA_HBCx10_Statecolored.pdf"), width = 7, height = 7)
...
pdf(file.path(Output_Figs_micro_RNA,"PCA_HBCx10_Modelcolored.pdf"), width = 7, height = 7)
...
pdf(file.path(Output_Figs_micro_RNA,"PCA_HBCx10_Treatmentcolored.pdf"), width = 7, height = 7)
...
pdf(file.path(Output_Figs_micro_RNA,"PCA_HBCx10_Clustercolored.pdf"), width = 7, height = 7)
...
pdf(file.path(Output_Figs_micro_RNA,"PCA_HBCx10_Dendrogramm.pdf"), width = 15, height = 15)
...
write.csv(df_pca, file.path(Output_Objects_micro_RNA, "DF_PCA_HBCx10.csv"), row.names = F)
```

Make PCA & Hierarchical Clustering for HBCx33
```{r}
#Load HBCx33 rawcount matrices
...
# Merge the matrices
microarray_mat = merge(HBCx33_chemonaive, HBCx33_Cisplatin, by = "Symbol", all = F)
...
# Elbow plot -> Take nPCs = 2 for Hierarchical Clustering 
pdf(file.path(Output_Figs_micro_RNA,"PCA_HBCx33_Elbowplot.pdf"), width = 7, height = 7)
fviz_eig(res.pca, ncp = 20) + theme_classic()
dev.off()
...
pdf(file.path(Output_Figs_micro_RNA,"PCA_HBCx33_Statecolored.pdf"), width = 7, height = 7)
df_pca %>% ggplot(aes(x = Dim.1, y = Dim.2, fill = state)) + 
  geom_point(size = 6, shape = 21, color = "black", alpha = 1) + 
  theme_classic() +
  xlab(paste0("Dim.1 (",df_pca_contrib[1], "%)")) + 
...
pdf(file.path(Output_Figs_micro_RNA,"PCA_HBCx33_Modelcolored.pdf"), width = 7, height = 7)
df_pca %>% ggplot(aes(x = Dim.1, y = Dim.2, fill = model)) + 
  geom_point(size = 6, shape = 21, color = "black", alpha = 1) + 
  theme_classic() +
  xlab(paste0("Dim.1 (",df_pca_contrib[1], "%)")) + 
...
pdf(file.path(Output_Figs_micro_RNA,"PCA_HBCx33_Treatmentcolored.pdf"), width = 7, height = 7)
df_pca %>% ggplot(aes(x = Dim.1, y = Dim.2, shape = treatment)) + 
  geom_point(size = 6,fill = "grey50", color = "black", alpha = 1) + 
  theme_classic() +
  xlab(paste0("Dim.1 (",df_pca_contrib[1], "%)")) + 
...
pdf(file.path(Output_Figs_micro_RNA,"PCA_HBCx33_Clustercolored.pdf"), width = 7, height = 7)
df_pca %>% ggplot(aes(x = Dim.1, y = Dim.2, shape = Cluster)) +
  geom_point(size = 6, alpha = 1, color = "black") + 
  theme_classic() +
  scale_shape_manual(values = AnnShape_Cluster) +
...
pdf(file.path(Output_Figs_micro_RNA,"PCA_HBCx33_Dendrogramm.pdf"), width = 15, height = 15)
ggplot(dend_data$segments) + 
  geom_segment(aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_text(data = dend_data$labels, aes(x, y, label = label, color = state), hjust = 1, angle = 90, size = 3) + 
  ylim(-100, 350) + 
...
write.csv(df_pca, file.path(Output_Objects_micro_RNA, "DF_PCA_HBCx33.csv"), row.names = F)
```
